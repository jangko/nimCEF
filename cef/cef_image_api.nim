...
type
  cef_image* = object of cef_base
    ...
    add_bitmap*: proc(self: ptr cef_image, scale_factor: cfloat,
      ...
      pixel_data_size: csize_t): cint {.cef_callback.}

    # Add a PNG image representation for |scale_factor|. |png_data| is the image
    # data of size |png_data_size|. Any alpha transparency in the PNG data will
    # be maintained.
    add_png*: proc(self: ptr cef_image, scale_factor: cfloat,
      png_data: cstring, png_data_size: csize_t): cint {.cef_callback.}

    # Create a JPEG image representation for |scale_factor|. |jpeg_data| is the
    # image data of size |jpeg_data_size|. The JPEG format does not support
    # transparency so the alpha byte will be set to 0xFF for all pixels.
    add_jpeg*: proc(self: ptr cef_image, scale_factor: cfloat,
      jpeg_data: cstring, jpeg_data_size: csize_t): cint {.cef_callback.}

    # Returns the image width in density independent pixel (DIP) units.
    get_width*: proc(self: ptr cef_image): csize_t {.cef_callback.}

    # Returns the image height in density independent pixel (DIP) units.
    get_height*: proc(self: ptr cef_image): csize_t {.cef_callback.}
