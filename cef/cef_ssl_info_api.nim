import cef_base_api, cef_values_api, cef_time_api
include cef_import
...
type
  ...
  cef_sslinfo* = object
    ...
    is_cert_status_error*: proc(self: ptr cef_sslinfo): cint {.cef_callback.}
    
    # Returns true (1) if the certificate status represents only minor errors
    # (e.g. failure to verify certificate revocation).
    is_cert_status_minor_error*: proc(self: ptr cef_sslinfo): cint {.cef_callback.}
