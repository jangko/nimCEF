...
type
  cef_read_handler* = object
    ...
    seek*: proc(self: ptr cef_read_handler, offset: int64,
        whence: cint): cint {.cef_callback.}
    
    # Return the current offset position.
    tell*: proc(self: ptr cef_read_handler): int64 {.cef_callback.}
    
    # Return non-zero if at end of file.
    eof*: proc(self: ptr cef_read_handler): cint {.cef_callback.}
  ...
  cef_stream_reader* = object
    ...
    # Return non-zero if at end of file.
    eof*: proc(self: ptr cef_stream_reader): cint {.cef_callback.}
  ...
  cef_write_handler* = object
    ...
    # Flush the stream.
    flush*: proc(self: ptr cef_write_handler): cint {.cef_callback.}
  ...
  cef_stream_writer* = object
    ...
    # Flush the stream.
    flush*: proc(self: ptr cef_stream_writer): cint {.cef_callback.}

    # Returns true (1) if this writer performs work like accessing the file
    # system which may block. Used as a hint for determining the thread to access
    # the writer from.
    may_block*: proc(self: ptr cef_stream_writer): cint {.cef_callback.}
