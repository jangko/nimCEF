import cef_base, cef_values
include cef_import

# Parse the specified |url| into its component parts. Returns false *(0) if the
# URL is NULL or invalid.
proc cef_parse_url*(url: ptr cef_string, parts: ptr cef_urlparts): int {.cef_import.}

# Creates a URL from the specified |parts|, which must contain a non-NULL spec
# or a non-NULL host and path *(at a minimum), but not both. Returns false *(0)
# if |parts| isn't initialized as described.
proc cef_create_url*(parts: ptr cef_urlparts, url: ptr cef_string): int {.cef_import.}
...
# The resulting string must be freed by calling cef_string_userfree_free*().
proc cef_uriencode*(text: ptr cef_string, use_plus: int): cef_string_userfree {.cef_import.}
...
# The resulting string must be freed by calling cef_string_userfree_free*().
proc cef_uridecode*(text: ptr cef_string, convert_to_utf8: int, 
...
proc cef_parse_csscolor*(str: ptr cef_string, strict: int, color: var cef_color): int {.cef_import.}
