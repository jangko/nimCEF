...
type
  ...
  cef_render_handler* = object
    ...
    get_root_screen_rect*: proc(self: ptr cef_render_handler,
      browser: ptr_cef_browser, rect: ptr cef_rect): int {.cef_callback.}
    ...
    get_screen_point*: proc(self: ptr cef_render_handler,
      browser: ptr_cef_browser, viewX, viewY: int, screenX, screenY: var int): int {.cef_callback.}
    ...
    get_screen_info*: proc(self: ptr cef_render_handler,
        browser: ptr_cef_browser, screen_info: ptr cef_screen_info): int {.cef_callback.}
  
    # Called when the browser wants to show or hide the popup widget. The popup
    # should be shown if |show| is true (1) and hidden if |show| is false (0).
    on_popup_show*: proc(self: ptr cef_render_handler,
        browser: ptr_cef_browser, show: int) {.cef_callback.}
    ...
    start_dragging*: proc(self: ptr cef_render_handler,
      ...
      allowed_ops: cef_drag_operations_mask, x, y: int): int {.cef_callback.}
