...
type
  ...
  cef_task_runner* = object
    ...
    # |that| object.
    is_same*: proc(self, that: ptr cef_task_runner): int {.cef_callback.}
  
    # Returns true (1) if this task runner belongs to the current thread.
    belongs_to_current_thread*: proc(self: ptr cef_task_runner): int {.cef_callback.}
  
    # Returns true (1) if this task runner is for the specified CEF thread.
    belongs_to_thread*: proc(self: ptr cef_task_runner,
       threadId: cef_thread_id): int {.cef_callback.}
  
    # Post a task for execution on the thread associated with this task runner.
    # Execution will occur asynchronously.
    post_task*: proc(self: ptr cef_task_runner,
      task: ptr cef_task): int {.cef_callback.}
    ...
    post_delayed_task*: proc(self: ptr cef_task_runner,
      task: ptr cef_task, delay_ms: int64): int {.cef_callback.}
...
proc cef_currently_on*(threadId: cef_thread_id): int {.cef_import.}

# Post a task for execution on the specified thread. Equivalent to using
# cef_task_tRunner::GetForThread(threadId)->PostTask(task).
proc cef_post_task*(threadId: cef_thread_id, task: ptr cef_task): int {.cef_import.}

# Post a task for delayed execution on the specified thread. Equivalent to
# using cef_task_tRunner::GetForThread(threadId)->PostDelayedTask(task,
# delay_ms).
proc cef_post_delayed_task*(threadId: cef_thread_id, task: ptr cef_task, delay_ms: int64): int {.cef_import.}