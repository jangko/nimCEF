...
type
  ...
  cef_zip_reader* = object
    ...
    move_to_first_file*: proc(self: ptr cef_zip_reader): int {.cef_callback.}

    # Moves the cursor to the next file in the archive. Returns true (1) if the
    # cursor position was set successfully.
    move_to_next_file*: proc(self: ptr cef_zip_reader): int {.cef_callback.}

    # Moves the cursor to the specified file in the archive. If |caseSensitive|
    # is true (1) then the search will be case sensitive. Returns true (1) if the
    # cursor position was set successfully.
    move_to_file*: proc(self: ptr cef_zip_reader,
      fileName: ptr cef_string, caseSensitive: int): int {.cef_callback.}

    # Closes the archive. This should be called directly to ensure that cleanup
    # occurs on the correct thread.
    close*: proc(self: ptr cef_zip_reader): int {.cef_callback.}
    ...
    open_file*: proc(self: ptr cef_zip_reader,
      password: ptr cef_string): int {.cef_callback.}

    # Closes the file.
    close_file*: proc(self: ptr cef_zip_reader): int {.cef_callback.}

    # Read uncompressed file contents into the specified buffer. Returns < 0 if
    # an error occurred, 0 if at the end of file, or the number of bytes read.
    read_file*: proc(self: ptr cef_zip_reader, buffer: pointer, bufferSize: csize): int {.cef_callback.}
  
    # Returns the current offset in the uncompressed file contents.
    tell*: proc(self: ptr cef_zip_reader): int64 {.cef_callback.}
  
    # Returns true (1) if at end of the file contents.
    eof*: proc(self: ptr cef_zip_reader): int {.cef_callback.}
