...
proc on_before_context_menu(self: ptr cef_context_menu_handler, browser: ptr_cef_browser,
  ...
  client.OnBeforeContextMenu(nc_wrap(brow), nc_wrap(frame), nc_wrap(params), model)
  release(brow)
  release(frame)
  release(params)
  release(model)
...
proc run_context_menu(self: ptr cef_context_menu_handler, browser: ptr_cef_browser,
  ...
  var client = get_client(browser)
  var brow = b_to_b(browser)
  result = client.RunContextMenu(nc_wrap(brow), nc_wrap(frame), nc_wrap(params), model, callback).cint
  release(brow)
  release(frame)
  release(params)
  release(model)
...
proc on_context_menu_command(self: ptr cef_context_menu_handler, browser: ptr_cef_browser,
  ...
  var client = get_client(browser)
  var brow = b_to_b(browser)
  result = client.OnContextMenuCommand(nc_wrap(brow), nc_wrap(frame), nc_wrap(params), command_id.cef_menu_id, event_flags).cint
  release(brow)
  release(frame)
  release(params)
