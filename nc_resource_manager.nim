...
proc GetNextValidProvider(self: NCResourceManager, pos: int): int =
  result = pos
  while result < self.providers.len:
    let pending = self.providers[result].deletionPending
...
proc OnBeforeResourceLoad*(self: NCResourceManager, browser: NCBrowser,
  ...
  let filteredURL = self.urlFilter(request.GetURL())
...
proc newDirectoryProvider(urlPath, directoryPath: string): DirectoryProvider =
  ...
  if directoryPath[directoryPath.len - 1] != DIR_SEP:
    result.directoryPath.add DIR_SEP
